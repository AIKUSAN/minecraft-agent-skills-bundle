...
set_count() {
  ...
  local current
  eval "current=\${$set_name}"
  printf '%s' "$current" | sed '/^$/d' | wc -l | tr -d ' '
}
...
check_json() {
  ...
  if jq empty "$file" >/dev/null 2>&1; then
    ...
    set_add INVALID_JSON_FILES "$file"
...
check_json_once() {
  local file="$1"
  if set_has VALIDATED_JSON_FILES "$file"; then
    return
  fi

  set_add VALIDATED_JSON_FILES "$file"
  check_json "$file"
...
should_validate_dimension_type_ref() {
  ...
  if [[ "$target_ns" == "minecraft" ]] && set_has VANILLA_DIMENSION_TYPES "$id"; then
    return 1
  fi
...
should_validate_noise_settings_ref() {
  ...
  if [[ "$target_ns" == "minecraft" ]] && set_has VANILLA_NOISE_SETTINGS "$id"; then
    return 1
  fi
...
if (( $(set_count PLACED_FEATURES) == 0 && (BIOME_FILES > 0 || BIOME_MODIFIER_FILES > 0) )); then
  warn "no placed_feature JSON files found"
fi

if (( $(set_count CONFIGURED_FEATURES) == 0 && $(set_count PLACED_FEATURES) > 0 )); then
  warn "no configured_feature JSON files found"
fi

echo "Checking dimension references..."
while IFS= read -r -d '' dimension_file; do
  if set_has INVALID_JSON_FILES "$dimension_file"; then
    continue
  fi
  ...
  if [[ -z "$type_ref" ]]; then
    ...
    if should_validate_dimension_type_ref "$ns" "$type_id"; then
      if set_has DIMENSION_TYPES "$type_id"; then
  ...
  if set_has NOISE_SETTINGS "$settings_id"; then
...
while IFS= read -r -d '' pf_file; do
  ...
  if set_has CONFIGURED_FEATURES "$feature_id"; then
...
while IFS= read -r -d '' ss_file; do
  ...
  while IFS= read -r sref; do
    ...
    if set_has STRUCTURES "$sid"; then
...
while IFS= read -r -d '' biome_file; do
  ...
  while IFS= read -r fref; do
    ...
    if set_has PLACED_FEATURES "$fid"; then
...
while IFS= read -r -d '' mod_file; do
  ...
  while IFS= read -r ref; do
    ...
    if set_has PLACED_FEATURES "$rid"; then
  ...
  while IFS= read -r ref; do
    ...
    [[ -z "$ref" ]] && continue
    rid="$(split_ref "$ns" "$ref")"
    if set_has STRUCTURES "$rid"; then
